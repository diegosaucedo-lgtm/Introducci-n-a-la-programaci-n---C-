...
void eliminarNodoHallar(Nodo *, int);
void eliminarNodo(Nodo *);
void reemplazarEliminar(Nodo *, Nodo *);
...
void eliminarNodoHallar(Nodo *arbol, int n) {
	...
	if (arbol == NULL) { //Si el arbol esta vacio
		...
		eliminarNodoHallar(arbol->izq,n); // Busca por la izq
	}
	else if (n > arbol->dato){ // Si el valor es mayor
		eliminarNodoHallar(arbol->der, n); // Busca por la der
	} 
	else {
		eliminarNodo(arbol); //Si ya encontraste el valor
	}
}
...
void eliminarNodo(Nodo *nodoEliminar) {
	...
	if (nodoEliminar->izq && nodoEliminar->der) { //Si el nodo es un nodo padre con dos hijos
	...
	else if (nodoEliminar->izq) { //Si el nodo es un nodo padre con un hijo izq
		
		reemplazarEliminar(nodoEliminar,nodoEliminar->izq);
		destruirNodo(nodoEliminar);
		
	}
	else if (nodoEliminar->der) { //Si el nodo es un nodo padre con un hijo der
		
		reemplazarEliminar(nodoEliminar,nodoEliminar->der);
		destruirNodo(nodoEliminar);
	} 
	else { //Si el nodo no tiene hijos
		
		reemplazarEliminar(nodoEliminar, NULL);
		destruirNodo(nodoEliminar);
...
//Hallar valor minimo a la izquierda
...
void reemplazarEliminar(Nodo *arbol, Nodo *nuevoNodo) {
	
	if (arbol->padre) {
		
		//arbol->padre hay que asignarle nuevo hijo
	...
	if (nuevoNodo) {
		//Procedemos a asignarle un nuevo padre
